...
import { Shirt, ChevronLeft, Search, Copy, Download, Loader, ChevronDown, ChevronUp, X, Plus, Trash2 } from 'lucide-react';
      ...
      <section className="grid-container" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
        {relatorioEquipes.filter(r => r.total_camisetas > 0).map(r => (
          ...
          >
            ...
            <p style={{ fontSize: '0.65rem', margin: 0, opacity: 0.6 }}>{r.total_pedidos} {r.total_pedidos === 1 ? 'pessoa' : 'pessoas'}</p>
          </div>
        ))}
      </section>
        ...
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {filteredPedidos.map(p => (
              <div key={p.id} className="card" style={{ padding: '1rem 1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', flexWrap: 'wrap' }}>

                <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', flex: '1 1 auto', flexWrap: 'wrap' }}>
                  <div style={{ minWidth: '220px', flex: '1 1 auto' }}>
                    <h3 style={{ fontSize: '0.95rem', margin: '0 0 0.25rem 0', fontWeight: 600 }}>{p.pessoa_nome}</h3>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', minWidth: '150px' }}>
                    <Shirt size={16} style={{ opacity: 0.5, color: 'var(--primary-color)' }} />
                    <span style={{ fontSize: '0.85rem', fontWeight: 500 }}>{p.camiseta_modelos?.nome}</span>
                  </div>

                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', minWidth: '120px' }}>
                    <span className="badge" style={{ backgroundColor: 'var(--surface-2)', padding: '0.2rem 0.5rem' }}>Tam: {p.tamanho}</span>
                    <span className="badge badge-primary" style={{ padding: '0.2rem 0.5rem' }}>{p.quantidade} un</span>
                  </div>
                </div>
