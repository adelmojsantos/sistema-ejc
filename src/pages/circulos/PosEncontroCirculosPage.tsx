...
export function PosEncontroCirculosPage() {
  ...
  const isFichaButtonEnabled = useMemo(() => {
    if (posEncontros.length === 0) return false;
    const otherPosPending = posEncontros.slice(0, -1).some(pos => {
      const status = realizacoesPorPos[pos.id]?.status ?? 'pendente';
      return status === 'pendente';
    });
    return !otherPosPending;
  }, [posEncontros, realizacoesPorPos]);
  ...
  return (
    <main className="main-content container fade-in" style={{ paddingBottom: '4rem' }}>
      <PageHeader
        ...
        actions={(isDetailRoute || isMediatorOnly) ? (
      ...
      <section className="pos-encontro-page">
        ...
        {!isDetailRoute && !isMediatorOnly && (
          <div className="card pos-encontro-filters">
            <div className="form-group pos-encontro-context-badge" style={{ margin: 0 }}>
              <span className="form-label">Edição ativa</span>
