...
export const labelDataService = {
  async listarPorEncontro(encontroId: string): Promise<LabelDataItem[]> {
    const { data, error } = await supabase
      .from('participacoes')
      .select('id, equipe_id, participante, coordenador, dados_confirmados, foto_url, pessoas(nome_completo, telefone, qr_code_token, observacoes_saude), equipes(nome, acesso_plenario), circulo_participacao(circulos(nome))')
