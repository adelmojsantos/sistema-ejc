-- Refine the active encounter readiness rules without changing the already
-- published migration. The response remains aggregate and contains no PII.
...
BEGIN
  ...
  v_basic_missing := array_remove(ARRAY[
    CASE WHEN btrim(COALESCE(v_encontro.nome, '')) = '' THEN 'Nome' END,
    CASE WHEN v_encontro.edicao IS NULL THEN 'Edição' END,
    CASE WHEN v_encontro.data_inicio IS NULL THEN 'Data de início' END,
    CASE WHEN v_encontro.data_fim IS NULL THEN 'Data de fim' END,
    CASE WHEN btrim(COALESCE(v_encontro.local, '')) = '' THEN 'Local' END,
    CASE WHEN btrim(COALESCE(v_encontro.tema, '')) = '' THEN 'Tema' END,
    CASE WHEN btrim(COALESCE(v_encontro.musica, '')) = '' THEN 'Música-tema' END
  ]::text[], NULL);

  v_fee_missing := array_remove(ARRAY[
    CASE WHEN COALESCE(v_encontro.valor_taxa, 0) <= 0 THEN 'Valor da taxa' END,
    CASE WHEN btrim(COALESCE(v_encontro.pix_taxa_chave, '')) = '' THEN 'Chave PIX' END,
    CASE WHEN btrim(COALESCE(v_encontro.pix_taxa_tipo, '')) = '' THEN 'Tipo da chave PIX' END
  ]::text[], NULL);

  v_metrics := v_metrics || jsonb_build_object(
    'basic_configured', cardinality(v_basic_missing) = 0,
    'basic_missing_fields', to_jsonb(v_basic_missing),
    'fee_configured', cardinality(v_fee_missing) = 0,
    'fee_missing_fields', to_jsonb(v_fee_missing),
