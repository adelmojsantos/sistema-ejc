-- Saves the operational visit as a single database transaction.
-- Uploads remain outside this function because object storage is not transactional.

BEGIN;

CREATE OR REPLACE FUNCTION public.salvar_visita_completa(
  p_visita_id uuid,
  p_dados jsonb
)
RETURNS jsonb
...
BEGIN
  ...
  FOR v_intencao IN SELECT value FROM jsonb_array_elements(COALESCE(p_dados -> 'intencoes', '[]'::jsonb))
  LOOP
    ...
    IF NOT EXISTS (SELECT 1 FROM public.camiseta_modelos WHERE id = (v_intencao ->> 'modelo_id')::uuid) THEN
    ...
    IF NULLIF(v_intencao ->> 'id', '') IS NOT NULL THEN
      ...
      END IF;
      v_intencao_ids := array_append(v_intencao_ids, (v_intencao ->> 'id')::uuid);
    ELSE
      INSERT INTO public.visita_intencao_camiseta (visita_id, modelo_id, tamanho, quantidade)
      VALUES (
        p_visita_id,
