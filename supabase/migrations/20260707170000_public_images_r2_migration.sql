CREATE TABLE IF NOT EXISTS public.public_image_r2_migracoes (
  source_bucket text NOT NULL,
  source_path text PRIMARY KEY,
  target_provider text NOT NULL DEFAULT 'cloudflare-r2',
...
ALTER TABLE public.public_image_r2_migracoes ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Admins inspect public image r2 migrations"
ON public.public_image_r2_migracoes;
CREATE POLICY "Admins inspect public image r2 migrations"
ON public.public_image_r2_migracoes
FOR SELECT
TO authenticated
USING (public.is_admin());

COMMENT ON TABLE public.public_image_r2_migracoes IS
'Auditoria da migração de imagens públicas do Supabase Storage para Cloudflare R2.';
...
'Atualiza referências de imagens públicas migradas para R2 e registra auditoria na mesma transação.';
