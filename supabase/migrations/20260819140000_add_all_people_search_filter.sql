-- Adiciona a busca agregada aos campos já permitidos, sem incluir comunidade.

BEGIN;

CREATE OR REPLACE FUNCTION public.search_pessoas_by_field(
  p_search_field text DEFAULT 'todos',
...
DECLARE
  ...
  v_result jsonb;
BEGIN
  IF v_field NOT IN ('todos', 'nome', 'email', 'telefone', 'endereco') THEN
      ...
      AND (
        ...
        OR CASE v_field
          WHEN 'todos' THEN
            ...
            OR lower(COALESCE(person.email, '')) LIKE v_email_pattern ESCAPE E'\\'
            OR (
              v_digits <> ''
              AND (
                regexp_replace(COALESCE(person.telefone, ''), '\D', '', 'g') LIKE '%' || v_digits || '%'
              )
            )
            OR unaccent(lower(concat_ws(
              ' ',
              person.endereco,
              person.numero,
              person.complemento,
...
'Busca pessoas em todos os campos permitidos ou em um campo específico, respeitando RLS, encontro, paginação e normalização adequada.';
